...
public class SkinnedMeshSine : MonoBehaviour
{
    ...
    [Tooltip(
        "Defines whether the Deformer is static or Dynamic, if true, the deformer will only be calculated once at Start")]
    ...
    void Start()
    {
        if (UseEffector != false)
        {
            ...
            {
                Debug.LogWarning(
                    "Please assign an effector to the effector Value, to create an effector go to: Mesh Deformer -> createEffector");
            }
        }
        ...
        {
            Debug.Log(
                "Please assign this script to a Skinned Mesh, you can find out which mesh is skinned by checking if it has a Skinned Mesh Renderer Attached to it");
            ISkinned = false;
        }
        else
        {
    ...
    void SineWaveEffector()
    {
        ...
        if (!InvertedEffector)
        {
            for (int i = 0; i < originalVertices.Length; i++)
            {
                if (Vector3.Distance(transform.TransformPoint(originalVertices[i]), Effector.transform.position) <=
                    EffectorDistance)
                {
                    float dist = Vector3.Distance(transform.TransformPoint(originalVertices[i]),
                    ...
                    switch (DeformAxis)
                    {
                        ...
                        case Axis.Y:
                            ...
                            if (!predictable)
                            {
                                new_y = y + PeakMultiplier *
                                    Mathf.Sin((Frequency * x + Phaze) * Mathf.PI * curveValue * EcurveValue);
                            }
                            else
                            {
                                new_y = y + PeakMultiplier * Mathf.Sin(Frequency * x + Phaze) * curveValue *
