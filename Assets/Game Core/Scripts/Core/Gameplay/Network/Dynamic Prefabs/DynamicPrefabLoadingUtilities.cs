...
namespace GameCore.Gameplay.Network.DynamicPrefabs
{
    ...
    public static class DynamicPrefabLoadingUtilities
    {
        ...
        static Dictionary<AddressableGUID, AsyncOperationHandle<GameObject>> s_LoadedDynamicPrefabResourceHandles = new(new AddressableGUIDEqualityComparer());

        static List<AddressableGUID> s_DynamicPrefabGUIDs = new(); //cached list to avoid GC

        //A storage where we keep the association between the dynamic prefab (hash of it's GUID) and the clients that have it loaded
        static Dictionary<int, HashSet<ulong>> s_PrefabHashToClientIds = new();
